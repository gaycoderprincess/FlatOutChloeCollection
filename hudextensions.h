namespace NewMenuHud {
	...
	const char* GetLoadingScreenForTrack(int track) {
		switch (track) {
	...
	uintptr_t OnLoadToMenuASM_jmp = 0x4C0610;
	void __attribute__((naked)) OnLoadToMenuASM() {
		__asm__ (
			"pushad\n\t"
			"mov ecx, ebx\n\t"
			"call %1\n\t"
	...
	uintptr_t OnLoadToRaceASM_jmp = 0x4C0610;
	void __attribute__((naked)) OnLoadToRaceASM() {
		__asm__ (
			"pushad\n\t"
			"mov ecx, ebx\n\t"
			"call %1\n\t"
